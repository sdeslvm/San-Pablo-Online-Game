...
class SanPabloWebContentLoader: ObservableObject {
    @Published var loadState: SanPabloWebLoadState = SanPabloWebLoadState.idle()
    let urlToLoad: URL
    ...
    private func setupProgressListener() {
        progressSubject
            ...
            .sink { [weak self] prog in
                ...
                if prog < 1.0 {
                    self.loadState = SanPabloWebLoadState.progress(prog)
                } else {
                    self.loadState = SanPabloWebLoadState.success()
                }
            }
    ...
    private func startLoading() {
        ...
        self.loadState = SanPabloWebLoadState.progress(0.0)
        webView.load(req)
    ...
    func setNetworkStatus(_ connected: Bool) {
        if connected && loadState.type == .offline {
            ...
            self.loadState = SanPabloWebLoadState.offline()
        }
    }
}
